...
export function createDeepSeekRoundGenerator(
  ...
  async function ask(
    ...
    for (let turn = 0; turn < MAX_TURNS; turn += 1) {
      ...
      if (submission) {
        try {
          return JSON.parse(submission.arguments) as unknown;
        } catch {
  ...
  return {
    async generate(request, signal) {
      ...
      for (let attempt = 1; attempt <= attempts; attempt += 1) {
        ...
        // always a file name that does not exist, and the names are the only
        // thing that says whether the model stuck to canonical files or made
        // some up.
        console.warn(
